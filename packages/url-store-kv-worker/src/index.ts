...
async function verifyAuth(c: Context<{ Bindings: Env }>): Promise<boolean> {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
        ...
        try {
            if (!c.env.LOGTO_JWKS_URL) return false;
            const jwks = createRemoteJWKSet(new URL(c.env.LOGTO_JWKS_URL));
...
app.get('/:key', async (c: Context) => {
    ...
    if (!key) return c.text("Hmmmmmmm...", 404);
